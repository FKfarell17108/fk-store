...
<div id="box">
  ...
  $query = $conn->prepare("SELECT * FROM tbl_bantuan LIMIT $posisi, $batas");
  $query->execute();
  $data = $query->fetchAll();
  ...
  <table class="news">
    <tr>
      ...
      <th style="text-align: center;">Id Bantuan</th>
    ...
    foreach ($data as $value): ?>
      <tr>
        ...
        <td>
        <a class="tombol-biru" href="mailto:<?php echo $value['email']; ?>"><i class='bx bx-envelope'></i></a>
        </td>
      </tr>
  ...
  $semua = $conn->prepare("SELECT * FROM tbl_bantuan");
  $semua->execute();
  ...
  for ($i = 1; $i <= $jmlhal; $i++) {
    if ($i == $hal) {
      ...
      echo "<span><a href='?page=bantuan&hal=$i'>$i</a></span>";
    }
  }
  if ($hal < $jmlhal) {
  }

...
<div id="box">
  ...
  $query = $conn->prepare("SELECT * FROM tbl_pengiriman LIMIT $posisi, $batas");
  $query->execute();
  $data = $query->fetchAll();
  ...
  <table class="news">
    <tr>
      ...
      <th style="text-align: center;">Id Pengiriman</th>
    ...
    foreach ($data as $value): ?>
      <tr>
        ...
        <td style="white-space: nowrap;"><?php echo $value['id_pengiriman'] ?></td>
        <td style="white-space: nowrap;"><?php echo $value['nama_penerima'] ?></td>
        <td style="white-space: nowrap;"><?php echo $value['alamat'] ?></td>
        <td style="white-space: nowrap;"><?php echo $value['nomor_telepon'] ?></td>
        <td style="white-space: nowrap;"><?php echo $value['kode_pos'] ?></td>
        <!-- <td>
  ...
  $semua = $conn->prepare("SELECT * FROM tbl_pengiriman");
  $semua->execute();
  ...
  for ($i = 1; $i <= $jmlhal; $i++) {
    if ($i == $hal) {
      ...
      echo "<span><a href='?page=pengiriman&hal=$i'>$i</a></span>";
    }
  }
  if ($hal < $jmlhal) {
  }

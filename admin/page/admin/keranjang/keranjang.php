...
<div id="box">
  ...
  $query = $conn->prepare("SELECT * FROM tbl_keranjang
                        JOIN tbl_users ON tbl_keranjang.id_user=tbl_users.id_user
                        LIMIT $posisi, $batas");
  $query->execute();
  $data = $query->fetchAll();
  ...
  <table class="news">
    ...
    foreach ($data as $value): ?>
      <tr>
        ...
        <td style="white-space: nowrap;"><?php echo $value['id'] ?></td>
        ...
        <td style="white-space: nowrap;"><?php echo "Rp. " . number_format($value['total'], 0, ',', '.') ?></td>
        <td style="white-space: nowrap;"><?php echo $value['date_in'] ?></td>
        <!-- <td>
  ...
  $semua = $conn->prepare("SELECT * FROM tbl_keranjang");
  $semua->execute();
  ...
  for ($i = 1; $i <= $jmlhal; $i++) {
    if ($i == $hal) {
      ...
      echo "<span><a href='?page=keranjang&hal=$i'>$i</a></span>";
    }
  }
  if ($hal < $jmlhal) {
  }
